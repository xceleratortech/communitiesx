...
// Community data with user assignments
const communityData = {
    //   "Aasta": [
    ...
    //   ],
    //   "Happy Age": [
    //     { "name": "Nachiket Vijay Hiredesai", "email": "hiredesaiahimsa3@gmail.com" },
    ...
    //     { "name": "NIKHEEL BHUMANNA SHIRSHYAD", "email": "nikheelshirashyad5074@gmail.com" },
    //     { "name": "NARAYANA R PUJARI", "email": "rvim23mba094.rvim@rvei.edu.in" },
    //     { "name": "Poornima Ganapati Hegde", "email": "poornimahegde389@gmail.com" },
    ...
    //   ]
    testComms: [
        { name: 'Ranjan-Atria', email: 'ranjan.sb@atriauniversity.edu.in' },
    ],
};
...
function createWelcomeEmail(name: string, email: string, password: string) {
    ...
    const LOGO_URL = 'https://bucket.xcelerator.co.in/TiE_LOGO.jpeg';
        ...
        <div class="container">
            <div class="header">
                <img src="${LOGO_URL}" 
                     ...
                     width="200"
                     height="auto">
                <h1>Welcome to TiE Communities!</h1>
            </div>
            ...
            <div class="content">
                ...
                <div style="text-align: center;">
                    <a href="${platformUrl}" class="login-button">🚀 Access Your Platform</a>
                </div>
...
async function createUser(name: string, email: string): Promise<string> {
    ...
    try {
        ...
        if (error) {
            console.log(`   ⚠️  Failed to send email to: ${email} - ${error}`);
        } else {
            console.log(
...
async function testEmail() {
    ...
    const testEmail = 'reachmrniranjan@gmail.com'; // Use your email for testing
    ...
    try {
        ...
        // Debug: Log the image tag to verify it's correct
        const imgMatch = emailTemplate.html.match(/<img[^>]*src="[^"]*"/);
        ...
        if (error) {
            console.log(
                `❌ Failed to send test email to: ${testEmail} - ${error}`,
            );
        } else {
            console.log(
                `✅ Test email sent successfully to: ${testEmail} (ID: ${data?.id})`,
            );
        }
    } catch (error) {
